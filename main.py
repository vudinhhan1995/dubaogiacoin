...
# --- AI Libraries (Chỉ giữ lại các thư viện cho LSTM) ---
...
MODEL_RETRAIN_INTERVAL = 86400 # 24 giờ
...
def get_historical_data(symbol, days=365): # Đã điều chỉnh về 365 ngày
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    now = time.time()
    try:
        ...
        # Nếu có cache hợp lệ (< 24h), dùng luôn
...
# ------------------------------------------------------------------
# 3. AI MODEL: LSTM ONLY
# ------------------------------------------------------------------

def calculate_forecast_lstm(symbol, history_prices, days_to_predict=7):
    """
    Mô hình Deep Learning LSTM (Đã tăng Epochs lên 50)
    """
    try:
        # 365 ngày dữ liệu là đủ cho window 60
        ...
        if should_train:
            ...
            # TĂNG EPOCHS LÊN 50 ĐỂ HỌC SÂU HƠN VỚI DỮ LIỆU 365 NGÀY
            model.fit(x_train, y_train, epochs=50, batch_size=32, verbose=0)
            
            model.save(model_path)
            model_status = "LSTM (New Training)"
            print(f"Trained new model for {symbol} (50 Epochs)")
...
def get_market_sentiment(pnl, history_prices, predicted_prices=None):
    ...
    if current_rsi > 70:
        advice.append(f"RSI cao ({current_rsi:.0f}).")
        if trend_score == 1:
            advice.append("Đà tăng mạnh nhưng rủi ro điều chỉnh.")
            color = "warning"
        elif trend_score == -1:
            advice.append("Cảnh báo ĐẢO CHIỀU GIẢM! Nên chốt lời.")
            color = "danger"
    elif current_rsi < 30:
        advice.append(f"RSI thấp ({current_rsi:.0f}).")
        if trend_score == 1:
            advice.append("Tín hiệu ĐẢO CHIỀU TĂNG. Cơ hội MUA tốt.")
            color = "success"
        else:
            advice.append("Giá đang giảm sâu.")
            color = "warning"
    else:
        advice.append(f"RSI trung tính ({current_rsi:.0f}).")
        if trend_score == 1:
            advice.append(f"AI dự báo {trend_msg}. Xu hướng tích cực.")
            color = "primary"
        elif trend_score == -1:
    ...
    if pnl < -15 and trend_score == 1:
        advice.append("Vị thế lỗ nhưng dự báo hồi -> Cân nhắc DCA.")
    elif pnl > 20 and trend_score == -1:
        advice.append("Đang lãi tốt & dự báo giảm -> Nên chốt lời.")
...
<head>
    ...
    <title>AI Crypto Portfolio (LSTM)</title>
    ...
    <style>
        ...
        .spinner-ai {
            ...
            border-top: 4px solid #6610f2; border-radius: 50%;
...
<div id="loading-overlay">
    ...
    <h5 class="text-primary fw-bold">Neural Network Training...</h5>
    <p class="text-muted">Đang tối ưu hóa LSTM (50 Epochs). Vui lòng đợi...</p>
</div>

<div class="container-fluid">
    <div class="row">
        <div class="col-md-2 sidebar d-none d-md-block">
            <h4 class="mb-4 text-center"><i class="fas fa-brain text-info"></i> LSTM Crypto</h4>
            <ul class="nav flex-column">
                ...
                <li class="nav-item mb-2"><a href="#addModal" data-bs-toggle="modal" class="nav-link text-white-50"><i class="fas fa-plus me-2"></i> Thêm Coin</a></li>
            </ul>
            <div class="mt-auto pt-5 text-center small text-secondary">
                <p>Deep Learning Core</p>
            </div>
        </div>
            ...
            <div class="row mb-4">
                <div class="col-md-4">
                    <div class="card bg-primary text-white p-3">
                        <div><h6 class="text-white-50">Tổng Tài Sản</h6><h3>${{ "{:,.2f}".format(total_value) }}</h3></div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="card {{ 'bg-success' if total_pnl >= 0 else 'bg-danger' }} text-white p-3">
                        <div><h6 class="text-white-50">PnL</h6><h3>{{ "+" if total_pnl >= 0 else "" }}{{ "{:,.2f}".format(total_pnl) }}$</h3></div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="card bg-dark text-white p-3">
                        <div><h6 class="text-white-50">Số Coin</h6><h3>{{ portfolio|length }}</h3></div>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="card">
                        <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                            <h5 class="m-0 fw-bold">Danh Mục</h5>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0">
                                <thead class="table-light">
                                    <tr>
                                        <th>Token</th><th>Số lượng</th><th>Giá TB</th><th>Giá</th><th>Tổng</th><th>PnL</th><th>Dự báo</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ url_for('predict', symbol=item.symbol) }}" 
                                               class="btn btn-sm btn-outline-primary" onclick="showLoading()">
                                               <i class="fas fa-chart-line"></i> LSTM
                                            </a>
                                            <a href="{{ url_for('delete_coin', id=item.id) }}" class="btn btn-sm text-danger ms-1"><i class="fas fa-times"></i></a>
                                        </td>
                                    </tr>
                                    {% else %}
                                    <tr><td colspan="7" class="text-center py-4 text-muted">Trống</td></tr>
                                    {% endfor %}
                                </tbody>
                            </table>
                        </div>
                    ...
                    <div class="card mt-4" id="chart-section">
                        <div class="card-header bg-white py-3">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <h5 class="m-0 fw-bold">{{ chart_symbol }} - Deep Learning Forecast</h5>
                                    <span class="badge {{ 'bg-success' if 'Cached' in model_status else 'bg-warning' }} text-dark">
                                        {{ model_status }}
                                    </span>
                                </div>
                                <div class="btn-group btn-group-sm">
                                    {% for d in [7, 30, 90] %}
                                    <a href="{{ url_for('predict', symbol=chart_symbol, days=d) }}" 
                                       class="btn btn-outline-dark {{ 'active' if days_selected == d else '' }}"
                                       onclick="showLoading()">{{ d }}D</a>
                                    {% endfor %}
                                </div>
                            </div>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="alert alert-{{ advice_color }} d-flex align-items-center mt-3">
                                <i class="fas fa-brain fs-4 me-3"></i>
                                <div><strong>AI Analysis:</strong> {{ advice_text }}</div>
                            </div>
                        </div>
                    </div>
                    {% endif %}
                ...
                <div class="col-lg-4">
                    <div class="card">
                        ...
                        <div class="card-body"><canvas id="allocationChart"></canvas></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="modal-dialog">
        <form method="POST" action="{{ url_for('add_coin') }}">
            <div class="modal-content">
                <div class="modal-header"><h5 class="modal-title">Thêm Coin</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body">
                    <div class="mb-3"><label>Mã (VD: BTC)</label><input type="text" name="symbol" class="form-control" required style="text-transform: uppercase"></div>
                    <div class="mb-3"><label>Số lượng</label><input type="number" step="any" name="quantity" class="form-control" required></div>
                    <div class="mb-3"><label>Giá mua ($)</label><input type="number" step="any" name="buy_price" class="form-control" required></div>
                </div>
...
<script>
    ...
    new Chart(ctxAlloc, {
        ...
        data: {
            ...
            datasets: [{ data: [{% for i in portfolio %}{{ i.total_val }},{% endfor %}], backgroundColor: ['#4e73df', '#1cc88a', '#36b9cc', '#f6c23e', '#e74a3b'] }]
        }
    });
    ...
    new Chart(ctxMain, {
        ...
        data: {
            labels: labels,
            datasets: [
                { label: 'Lịch sử (365d)', data: histData, borderColor: '#858796', borderWidth: 1.5, pointRadius: 0, tension: 0.1 },
                { label: 'Dự báo LSTM', data: predSet, borderColor: '#6610f2', backgroundColor: 'rgba(102, 16, 242, 0.05)', borderWidth: 2, pointRadius: 0, tension: 0.3 }
            ]
        },
        options: {
            responsive: true,
...
def index():
    ...
    total_val = 0; total_pnl = 0; total_inv = 0
...
def predict(symbol):
    ...
    symbols = [r['symbol'] for r in rows]
    prices = get_current_prices_bulk(symbols)
    total_val = 0; total_inv = 0; target_pnl = 0
    ...
    # 365 ngày dữ liệu cho CoinGecko miễn phí
    ...
    # Chỉ dùng LSTM
    pred_lbl, pred_mean, _, _, model_status = calculate_forecast_lstm(symbol, hist_data, days)
...
if __name__ == '__main__':
    init_db()
    print("LSTM Crypto App running on http://127.0.0.1:5005")
